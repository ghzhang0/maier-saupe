...
def plot_defect(m0: np.ndarray, cone: ms.Cone, p: int, ax: matplotlib.axes.Axes,
    ...
    * Plot 1. p-atic texture, and 2. lattice sites colored by vorticity. (Gray:
    ...
    for n in range(N):
        ...
        xs = [cone.lattice.real[n] + hx*np.cos(m0[n]+2*np.pi/p*i)
              for i in range(p)]
        ys = [cone.lattice.imag[n] + hx*np.sin(m0[n]+2*np.pi/p*i)
              for i in range(p)]
        ...
        # Leave out apex's nearest neighbors (apex orientation not well-defined)
            ...
            angles = np.angle(cone.lattice[nn_inds] -
                              cone.lattice[n]) % (2 * np.pi)
            nang_inds = nn_inds[np.argsort(angles)]
            # Modulo all orietation angles by 2 pi / p
            nn_m0 = m0[nang_inds] % (2 * np.pi / p)
            ...
            for i in range(len(nn_m0)-1):
                ...
                poss = [nn_m0[i+1] - nn_m0[i] + k * 2 * np.pi / p for k
            ...
            poss = [nn_m0[0] - nn_m0[-1] + k * 2 * np.pi /p for k
                    in np.arange(-int(p/2)-1, int(p/2)+2)]
            dth = poss[np.argsort(np.abs(poss))[0]]
            diffs.append(dth)
            # Summing up \nabla \vartheta
            nns = np.sum(diffs)/(2 * np.pi / p)
            vorticity[n] = nns
            rgba = cmap(nns)
...
def hyp_plot(m0: np.ndarray, hyp: ms.Hyperbolic, p: int,
    ...
    for i in range(p):
        ax[0, 0].quiver(hyp.top.lattice.real, hyp.top.lattice.imag,
                        np.cos(m0[:len(hyp.top.lattice)]+i*2*np.pi/p),
                        np.sin(m0[:len(hyp.top.lattice)]+i*2*np.pi/p),
                        headwidth=0.5, scale=35, pivot='middle')
        ax[1, 0].quiver(hyp.bottom.lattice.real[:-hyp.r],
                        hyp.bottom.lattice.imag[:-hyp.r],
                        np.cos(m0[len(hyp.top.lattice):]+i*2*np.pi/p)[:-hyp.r],
                        np.sin(m0[len(hyp.top.lattice):]+i*2*np.pi/p)[:-hyp.r],
                        headwidth=0.5, scale=35, pivot='middle')
...
def plot_hyperbolic_seam(m0: np.ndarray, hyp: ms.Hyperbolic, p: int,
    ...
    for n in seam_inds:
        ...
        xs = [hyp.lattice.real[n] + hx*np.cos(m0[n]+2*np.pi/p*i)
              for i in range(p)]
        ys = [hyp.lattice.imag[n] + hx*np.sin(m0[n]+2*np.pi/p*i)
              for i in range(p)]
        ...
        if (np.abs(hyp.lattice[n]) < 1.1
            ...
            nn_inds = np.array(nn_inds)
            # Sort neighbors in CCW direction relative to center site
            angles = np.angle(hyp.lattice[nn_inds] - hyp.lattice[n])%(2*np.pi)
            nang_inds = nn_inds[np.argsort(angles)]
            # Modulo all orietation angles by 2 pi / p
            nn_m0 = m0[nang_inds]%(2*np.pi/p)
            diffs = []
